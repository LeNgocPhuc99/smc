...
contract DiamondLoupe is IDiamondLoupe {
    ...
    function facets() external view override returns (Facet[] memory facets_) {
        ...
        do {
            ...
            address facetAddressForSelector = ds
            ...
            for (uint256 index; index < numFacets; index++) {
                if (facets_[index].facetAddress == facetAddressForSelector) {
                    isNewFacet = false;
                    facets_[index].functionSelectors[
        ...
        returns (bytes4[] memory facetFunctionSelectors_)
    {
        ...
        for (uint256 i; i < ds.selectors.length; i++) {
            if (ds.selectorTofacet[ds.selectors[i]].facetAddress == _facet) {
        ...
        returns (address[] memory facetAddresses_)
    {
        ...
        uint256 numFacets;
        for (uint256 i; i < ds.selectors.length; i++) {
            address factAddress = ds
                .selectorTofacet[ds.selectors[i]]
                .facetAddress;
            bool addFacet = true;
    ...
    {
        LibDiamond.DiamondStorage storage ds = LibDiamond.getDiamondStorage();
        facetAddress_ = ds.selectorTofacet[_functionSelector].facetAddress;
    }
}
